...
import re
from litemp import LitempSyntaxError, Litemp
...
class LitempTest(TestCase):
    ...
    def assertSynErr(self, msg):
        pat = "^" + re.escape(msg) + "$"
        return self.assertRaisesRegexp(LitempSyntaxError, pat)

    def test_passthrough(self):
        # Strings without variables are passed through unchanged.
        self.assertEqual(Litemp("Hello").render(), "Hello")
        self.assertEqual(
            Litemp("Hello, 20% fun time!").render(),
            "Hello, 20% fun time!"
            )
    ...
    def test_reusability(self):
        # A single Litemp can be used more than once with different data.
        globs = {
            'upper': lambda x: x.upper(),
            'punct': '!',
            }

        template = Litemp("This is {{name|upper}}{{punct}}", globs)
